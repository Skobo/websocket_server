...
struct room_node* RoomHeadNode = NULL;
struct room_node* RoomCurrentNode = NULL;
...
int regexp_pd(char *buf,const char *pattern){
    ...
    //下面这个循环是输出匹配结果的，如需要，这里暂时不需要所以注释掉
...
/*
	...
	char *buf  要处理的源字符串
	const char *pattern 正则表代式字符串
	regmatch_t pmatch[1] 返回的处理结果

 */
 //void regexp(char *buf,const char *pattern,char **val);
  int regexp(char *bematch,const char *pattern,char **val){
            ...
            for( i=0;i<10 && pmatch[i].rm_so != -1; i++){
               // printf("%d\n",i);
                ...
                if(len){
                    ...
                    val[j]=(char*)malloc(len*sizeof(char));
                    memcpy(val[j],match,len);
                    //printf("%s===%d\n",val[j]);
                    //printf("%s==%d\n",match,j);
                    j++;
                }
 ...
 * 功能: 判断链表里面是否有房间号，
 * 形参: roomid 房间号
 * 返回: >如果有房间号返回最后一个节点指针 ，如果没有返回Null
 * 说明: 无
 ******************************************************************************/
 struct room_node* pdroom(int room,struct room_node* rh){
     struct room_node* dq = RoomHeadNode;
     while(dq != NULL){
         if(dq->room == room){
                rh = dq ;
                break;
...
//创建房间及其名下的子节点
struct room_node* create_room_node(int room,int fd,int id,char *username){
        struct room_node* c = (struct room_node*)malloc(sizeof(struct room_node));
        c->room = room;
        c->user_node_head =  (struct user_node*)malloc(sizeof(struct user_node));
        c->user_node_tail = c->user_node_head;//将房间最后一个节点指向刚刚创建的节点
        //将用户添加到对应的房间
        c->user_node_head->fd = fd;
        c->user_node_head->id = id;
        c->user_node_head->room = room;
        c->user_node_head->username=username;
        c->user_node_head->next = NULL;
        c->next = NULL;
        return c;
 }

 //输出房间及其名下的所有用户
 void printf_room_node_user(struct room_node* p){
     struct user_node* prnu;
     struct room_node* prrn = p;
     while(prrn != NULL){
         printf("=====fd====id====房间(%d)====用户名====\n",prrn->room);
         prnu = prrn->user_node_head;
         while(prnu != NULL){
             printf("----%d----%d----%d----%s----\n",prnu->fd,prnu->id,prnu->room,prnu->username);
             prnu = prnu->next;
         }
         prrn = prrn->next;
     }
...
int webSocket_serverLinkToClient(int fd, char *recvBuf, int bufLen)
{
    ...
    //printf("%s\n",recvBuf);
    const char *patt = "room=[0-9]*&id=[0-9]*&username=.*HTTP";
    r = regexp_pd(recvBuf,patt);//判断第一个正则是否成立
    if(r<0){
          return -10;
    }
   
    const char *pattern = "room=([0-9]*)&id=([0-9]*)&username=(.*)HTTP";
	char *val[4]={0};
    int lnum = 4;

	regexp(recvBuf,pattern,val);
    if(!val){
        //printf("正则匹配失败\n");
        return -10;
    }
    ...
    if( RoomHeadNode == NULL){
        RoomHeadNode = create_room_node(atoi(val[1]), fd,atoi(val[2]),val[3]);
        RoomCurrentNode = RoomHeadNode;
    }else{
        pdroom(atoi(val[1]),rh);
        if(rh == NULL){
            RoomCurrentNode->next = create_room_node(atoi(val[1]), fd,atoi(val[2]),val[3]);
            RoomCurrentNode = RoomCurrentNode->next;
        }else{
            struct user_node* ud =  (struct user_node*)malloc(sizeof(struct user_node));
            ud->fd = fd;
            ud->id = atoi(val[2]);
            ud->room = atoi(val[1]);
            ud->username=val[3];
            ud->next = NULL;
            rh->user_node_tail->next  = ud;
            rh->user_node_tail =  ud;
        }
    }
...
int webSocket_recv(int fd, char *data, int dataMaxLen, WebsocketData_Type *dataType)
{
    ...
    //数据可能超出了范围限制
    if(ret == dataMaxLen){
        printf("webSocket_recv : 数据包超出限制!! (recv/%d)\r\n", ret);
    }
    if(ret > 0)
    {
        //---------- websocket数据解包 ----------
        ...
        else //正常收包
        {
            ...
            {
                printf("webSocket_recv :数据包超出限制!! (recvPackage/%d)\r\n", retLen);
